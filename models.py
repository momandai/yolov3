...
def create_modules(module_defs):
    ...
    for mdef in module_defs:
        ...
        if mdef['type'] == 'convolutional':
        ...
        elif mdef['type'] == 'mobilenet-v2-bottleneck':
            t = int(mdef['t'])
            filters = int(mdef['filters'])
            n = int(mdef['n'])
            s = int(mdef['stride'])
            ...
            for j in range(n):
                if j == 0:
                    modules.add_module('mobilenet-v2-bottleneck_%d' % j, InvertedResidual(input_channel, output_channel, s, expand_ratio=t))
                else:
                    modules.add_module('mobilenet-v2-bottleneck_%d' % j, InvertedResidual(input_channel, output_channel, 1, expand_ratio=t))
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, img_size, var=None):
        ...
        if self.training:
            ...
            stride = self.tensor_stride.repeat((1, self.na * self.nx * self.ny, 2))
            ...
            xy = (torch.sigmoid(p[..., 0:2]) + grid_xy) * stride  # x, y
            wh = torch.exp(p[..., 2:4]) * anchor_wh * stride  # width, height
            p_conf = torch.sigmoid(p[..., 4:5])  # Conf
            p_cls = p[..., 5:5 + self.nc]
            ...
            # tmp = torch.cat((xy / ngu, wh, p_conf, p_cls), 2).squeeze().t()
            return torch.cat((xy, wh, p_conf, p_cls), 2).squeeze().t()
...
def create_grids(self, img_size=416, ng=(13, 13), device='cpu', type=torch.float32):
    ...
    self.tensor_stride = torch.Tensor((self.stride,)).to(device)
    self.nx = nx
    self.ny = ny
